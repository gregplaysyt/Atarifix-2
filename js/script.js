            ...
            <div class="service-card-content">
                ...
                <div class="service-actions">
                    ...
                    <button class="btn-read-less" style="display: none;">
                        Show Less <span style="margin-left: 8px;">▲</span>
                    </button>
                </div>
            </div>
        `;
...
function animateStats() {
    ...
    if (document.querySelector('.hero-stats')) {
        const heroStat1 = document.querySelector('.hero-stats .stat:nth-child(1) .stat-number');
        const heroStat2 = document.querySelector('.hero-stats .stat:nth-child(2) .stat-number');
        const heroStat3 = document.querySelector('.hero-stats .stat:nth-child(3) .stat-number');
        
        if (heroStat1) heroStat1.setAttribute('data-target', '50');
        if (heroStat2) heroStat2.setAttribute('data-target', '100');
        if (heroStat3) heroStat3.setAttribute('data-target', '24');
    }
    ...
    if (document.querySelector('.stats-grid')) {
        ...
        if (statBoxes.length >= 4) {
            const targets = ['50', '100', '24', '5'];
            const suffixes = ['+', '%', '/7', '+'];
            
            statBoxes.forEach((box, index) => {
                const statNumber = box.querySelector('.stat-number');
                if (statNumber) {
                    statNumber.setAttribute('data-target', targets[index]);
                    statNumber.setAttribute('data-suffix', suffixes[index]);
                    statNumber.innerText = '0' + suffixes[index];
                }
            });
        }
    }
...
function startCountingAnimation() {
    const statNumbers = document.querySelectorAll('.hero-stats .stat-number, .stats-grid .stat-number');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Start stats animation when stats come into view
    const statsSection = document.querySelector('.stats, .hero-stats');
    if (statsSection) {
